...
const januaryMonth = (fundStart, sdlc) => {
	const obj = {
		...
		moneyOut: function() {
			return (
				this.droidProduced() * this.droidProductionCost() +
				this.numberOfBugs() * this.bugFixCost()
			);
		},
		moneyIn: function() {
			return (this.droidProduced() - this.defectiveCount()) * this.droidSellCost();
		},
		droidProduced: function() {
			return getRandomInt(sdlc.droidProductionRate.lower, sdlc.droidProductionRate.upper);
		},
		defectivePercentage: function() {
			return (
				getRandomInt(
					sdlc.percentageOfDefectiveDroids.lower * 100,
					sdlc.percentageOfDefectiveDroids.upper * 100
				) / 100
			);
		},
		defectiveCount: function() {
			return this.droidProduced() * (this.defectivePercentage() / 100);
		},
		droidProductionCost: function() {
			return getRandomInt(sdlc.droidProductionCost.lower, sdlc.droidProductionCost.upper);
		},
		numberOfBugs: function() {
			return (
				(this.droidProductionCost() *
					getRandomInt(
						(sdlc.rateOfNewBugs.lower / 100) * 100,
						(sdlc.rateOfNewBugs.upper / 100) * 100
					)) /
				100
			);
		},
		...
		droidUpSellCost: function() {
			return getRandomInt(
				(sdlc.droidUpSellPercentage.lower / 100) * 100,
				(sdlc.droidUpSellPercentage.upper / 100) * 100
			);
		},
		droidSellCost: function() {
			return (
				this.droidProductionCost() * (this.droidUpSellCost() / 100) +
				this.droidProductionCost()
			);
		},
	};
...
const secondScreenSubmitHandler = handler => {
	...
	if (
		plaining == 0 ||
		defineRequirements == 0 ||
		designPrototyping == 0 ||
		softwaredevelopment == 0 ||
		operationMaintenance == 0 ||
		development == 0 ||
		testing == 0
	)
		return;
...
const upperLowerLimitCalc = (sprintObject, sprint) => {
	...
	const rate =
		(sprintObject.plaining * (-1 / 100) +
			sprintObject.defineRequirements * (0 / 100) +
			sprintObject.designPrototyping * (-5 / 100) +
			sprintObject.softwaredevelopment * (-5 / 100) +
			sprintObject.testing * (-10 / 100) +
			sprintObject.development * (2 / 100) +
			sprintObject.operationMaintenance * (-5 / 100)) *
		(potencyFactor / 100) *
		100;
	const cost =
		(sprintObject.plaining * (-1 / 100) +
		...
		100;
	const droidpr =
		(sprintObject.plaining * (2 / 100) +
			sprintObject.defineRequirements * (4 / 100) +
			sprintObject.designPrototyping * (5 / 100) +
			sprintObject.softwaredevelopment * (10 / 100) +
			sprintObject.testing * (2 / 100) +
			sprintObject.development * (5 / 100) +
			sprintObject.operationMaintenance * (5 / 100)) *
		(potencyFactor / 100) *
		100;
	const percentageodd =
		(sprintObject.plaining * (-1 / 100) +
			sprintObject.defineRequirements * (-5 / 100) +
			sprintObject.designPrototyping * (-5 / 100) +
			sprintObject.softwaredevelopment * (-1 / 100) +
			sprintObject.testing * (-5 / 100) +
			sprintObject.development * (-1 / 100) +
			sprintObject.operationMaintenance * (-1 / 100)) *
		(potencyFactor / 100) *
		100;
	const droidpc =
		(sprintObject.plaining * (-2 / 100) +
			...
			sprintObject.testing * (5 / 100) +
			sprintObject.development * (5 / 100) +
			sprintObject.operationMaintenance * (10 / 100)) *
		(potencyFactor / 100) *
		100;
	const droidsp =
		(sprintObject.plaining * (2 / 100) +
			sprintObject.defineRequirements * (5 / 100) +
			sprintObject.designPrototyping * (5 / 100) +
			sprintObject.softwaredevelopment * (8 / 100) +
			sprintObject.testing * (0 / 100) +
			sprintObject.development * (0 / 100) +
			sprintObject.operationMaintenance * (0 / 100)) *
		(potencyFactor / 100) *
		100;
	...
	let phases = {
		...
		costToFixBugs: {
			...
			lower: function() {
				return this.baseLine * ((-1 * this.adjustmentPercentage) / 100);
			},
			upper: function() {
				return this.baseLine;
			},
		...
		droidProductionRate: {
			...
			upper: function() {
				return this.baseLine * (this.adjustmentPercentage / 100) + this.baseLine;
			},
		},
		percentageOfDefectiveDroids: {
			baseLine: 5,
		...
		droidUpSellPercentage: {
			...
			upper: function() {
				return (this.baseLine / 100 + this.adjustmentPercentage / 100) * 100;
			},
		},
		droidUpSellCost: {
			baseLine: function() {
				return (
					(phases.droidProductionCost.baseLine * phases.droidUpSellPercentage.baseLine) /
						100 +
					phases.droidProductionCost.baseLine
				);
			},
			adjustmentPercentage: null,
			lower: function() {
				return (
					phases.droidProductionCost.lower() *
						(phases.droidUpSellPercentage.lower() / 100) +
					phases.droidProductionCost.lower()
				);
			},
			upper: function() {
				return (
					phases.droidProductionCost.upper() *
						(phases.droidUpSellPercentage.upper() / 100) +
					phases.droidProductionCost.upper()
				);
			},
		},
  ...
  `;
	...
	const html = `
  <div><b>Rate of new bugs</b>  <p>${sdlc.rateOfNewBugs.lower.toFixed(
		0
  )}%</p> <p>${sdlc.rateOfNewBugs.upper.toFixed(0)}%</p> </div>
  <div><b>Cost to fix bugs</b>  <p>R.${sdlc.costToFixBugs.lower.toFixed(
		0
  )}</p> <p>R.${sdlc.costToFixBugs.upper.toFixed(0)}</p> </div>
  <div><b>Droid production rate</b>  <p>${sdlc.droidProductionRate.lower.toFixed(
		0
  )}</p> <p>${sdlc.droidProductionRate.upper.toFixed(0)}</p> </div>
  <div><b>Percentage of defective droids</b>  <p>${sdlc.percentageOfDefectiveDroids.lower.toFixed(
		0
  )}%</p> <p>${sdlc.percentageOfDefectiveDroids.upper.toFixed(0)}%</p> </div>
  <div><b>Droid production cost</b>  <p>${sdlc.droidProductionCost.lower.toFixed(
		0
  )}</p> <p>${sdlc.droidProductionCost.upper.toFixed(0)}</p> </div>
  <div><b>Droid upsell percentage</b>  <p>${sdlc.droidUpSellPercentage.lower.toFixed(
		0
  )}%</p> <p>${sdlc.droidUpSellPercentage.upper.toFixed(0)}%</p> </div>
  <div><b>Droid sell cost</b>  <p>R.${sdlc.droidUpSellCost.lower.toFixed(
		0
  )}</p> <p>R.${sdlc.droidUpSellCost.upper.toFixed(0)}</p> </div>
  `;
...
const thirdScreenMonthsHandler = (remainingValue, handler) => {
	...
	const octorber = othersMotnhs(
		september.fundStart - september.moneyOut + september.moneyIn,
		sdlc
	);
	...
	const monthsData = [
		{ month: january, str: "January" },
		{ str: "Febuary", month: febuary },
		{ month: march, str: "March" },
		{ str: "April", month: aprial },
		{ month: may, str: "May" },
		{ month: june, str: "June" },
		{ month: july, str: "July" },
		{ month: august, str: "August" },
		{ month: september, str: "September" },
		{ month: octorber, str: "October" },
		{ month: november, str: "November" },
		{ month: december, str: "December" },
	];
	...
	btn.addEventListener("click", () => {
		...
		if (checkShowMonth === 12 || checkShowMonth > 12) {
			...
			tableBody.innerHTML = listMonths(
				monthsData[checkShowMonth].month,
				monthsData[checkShowMonth].str
			);
		}
	});
};
...
const modulesHandler = () => {
	if (screen == 1) {
		...
		// return secondScreenHandler(screenAdd, 20)
	} else if (screen == 3) {
		wrapper.innerHTML = "";
